...
def EC_at_RGB_value(contributions, r, g):
...
def difference_RGB_ECP(ecp_1, ecp_2, return_contributions = False):
    ...
    contributions = [((fmin, fmin), 0),
                     ((fmax, fmax), 0)]
    
    contributions += ecp_1
    contributions += [(c[0], -1*c[1]) for c in ecp_2]
    
    contributions = [((fmin, fmin), 0)]+prune_contributions(contributions)+ \
                    [((fmax, fmax), 0)]
...
#     print(contributions)
    ...
    for i, r in enumerate(R_list[:-1]):
        ...
        for j, g in enumerate(G_list[:-1]):
            ...
            difference += abs(EC_at_RGB_value(contributions, r, g) * delta_r * delta_g)
